...
such as I/O operation and bloc ksizes have to be revisited.
...
over 300 TB of dis kstorage, and are heavily accessed by
...
the file rather than go bac kand forth.
...
mium on processing data in bul kat a high rate, while
...
names. We support the usual operations tocreate,delete,
open,close,read,andwritefiles.
Moreover, GFS hassnapshotandrecord append opera-
...
2.3 Architecture
A GFS cluster consists of a singlemasterand multiple
chunkserversand is accessed by multipleclients,asshown
...
Files are divided into fixed-sizechunks. Each chun kis
identified by an immutable and globally unique 64 bitchunk
handleassigned by the master at the time of chun kcreation.
Chunkservers store chunks on local disks as Linux files and
read or write chun kdata specified by a chun khandle and
byte range. For reliability, each chun kis replicated on multi-
...
It also controls system-wide activities such as chun klease
management, garbage collection of orphaned chunks, and
chun kmigration between chun kservers. The master peri-
odically communicates with each chunkserver inHeartBeat
...
therefore need not hoo kinto the Linux vnode layer.
...
ence to Figure 1. First, using the fixed chun ksize, the client
translates the file name and byte offset specified by the ap-
plication into a chun kindex within the file. Then, it sends
...
information using the file name and chun kindex as the key.
...
of the same chun krequire no more client-master interaction
...
2.5 Chunk Size
Chun ksize is one of the key design parameters. We have
chosen 64 MB, which is much larger than typical file sys-
tem bloc ksizes. Each chun kreplica is stored as a plain
...
a large chun ksize.
A large chun ksize offers several important advantages.
First, it reduces clients’ need to interact with the master
because reads and writes on the same chun krequire only
one initial request to the master for chun klocation informa-
...
comfortably cache all the chun klocation information for a
...
On the other hand, a large chun ksize, even with lazy space
...
and chun knamespaces, the mapping from files to chun ks,
and the locations of each chunk’s replicas. All metadata is
kept in the master’s memory. The first two types (names-
paces and file-to-chun kmapping) are also kept persistent by
logging mutations to anoperation logstored on the mas-
ter’s local dis kand replicated on remote machines. Using
a log allows us to update the master state simply, reliably,
and without risking inconsistencies in the event of a master
crash. The master does not store chun klocation informa-
...
This periodic scanning is used to implement chun kgarbage
collection, re-replication in the presence of chunkserver fail-
ures, and chun kmigration to balance load and dis kspace
...
chun kplacement and monitors chun kserver status with reg-
ularHeartBeatmessages.
...
completed, it is written to dis kboth locally and remotely.
...
rizes the result. A file region isconsistentif all clients will
...
mutations to a chun kin the same order on all its replicas
(Section 3.1), and (b) using chun kversion numbers to detect
...
metadata of a chun ksuch as a write or an append opera-
tion. Each mutation is performed at all the chunk’s replicas.
We use leases to maintain a consistent mutation order across
replicas. The master grants a chun klease to one of the repli-
cas, which we call theprimary. The primary picks a serial
...
The client asks the master which chunkserver holds
the current lease for the chun kand the locations of
the other replicas. If no one has a lease, the master
grants one to a replica it chooses (not shown).
The master replies with the identity of the primary and
...
wor ktopology regardless of which chun kserver is the
...
through (7) before falling bac kto a retry from the be-
...
use the networ kefficiently. While control flows from the
...
machine’s networ kbandwidth, avoid networ kbottlenec ks
and high-latency links, and minimize the latency to push
through all the data.
To fully utilize each machine’s networ kbandwidth, the
...
networ ktopology that has not received it. Suppose the
...
is closer to S2, and so on. Our networ ktopology is simple
...
cially helpful to us because we use a switched networ kwith
full-duplex links. Sending the data immediately does not
reduce the receive rate. Without networ kcongestion, the
ideal elapsed time for transferringBbytes toRreplicas is
B/T+RLwhereTis the networ kthroughput andLis la-
...
ing to a file opened inOAPPENDmode in Unix without the
...
ample through a distributed loc kmanager, if they do so
...
last chun kof the file Then, it sends its request to the pri-
mary. The primary checks to see if appending the record
to the current chun kwould cause the chun kto exceed the
maximum size (64 MB). If so, it pads the chun kto the max-
...
most one-fourth of the maximum chun ksize to keep worst-
...
operation. As a result, replicas of the same chun kmay con-
...
higher offset or a different chun keven if a different replica
...
can later be committed or rolled bac keasily.
...
/d1/d2/.../dn, and either a read loc kor a write loc kon the
full pathname/d1/d2/.../dn/leaf.Notethatleafmay be
a file or directory depending on the operation.
We now illustrate how this locking mechanism can prevent
afile/home/user/foofrom being created while/home/user
is being snapshotted to/save/user. The snapshot oper-
ationacquiresreadlockson/homeand/save,andwrite
locks on/home/userand/save/user. The file creation ac-
quires read locks on/homeand/home/user,andawrite
loc kon/home/user/foo. The two operations will be seri-
...
The read loc kon the name is sufficient to protect the parent
...
same directory: each acquires a read loc kon the directory
name and a write loc kon the file name. The read loc kon
...
ally, bandwidth into or out of a rac kmay be less than the
...
The chun kreplica placement policy serves two purposes:
maximize data reliability and availability, and maximize net-
wor kbandwidth utilization. For both, it is not enough to
spread replicas across machines, which only guards against
dis kor machine failures and fully utilizes each machine’s net-
wor kbandwidth. We must also spread chun kreplicas across
racks. This ensures that some replicas of a chunk will sur-
vive and remain available even if an entire rac kis damaged
or offline (for example, due to failure of a shared resource
like a network switch or power circuit). It also means that
traffic, especially reads, for a chun kcan exploit the aggre-
...
Chun kreplicas are created for three reasons: chun kcre-
ation, re-replication, and rebalancing.
When the mastercreatesa chunk, it chooses where to
place the initially empty replicas. It considers several fac-
tors. (1) We want to place new replicas on chunkservers with
below-average dis kspace utilization. Over time this will
equalize dis kutilization across chun kservers. (2) We want to
...
want to spread replicas of a chun kacross rac ks.
The masterre-replicatesa chun kas soon as the number
...
goal is increased. Each chun kthat needs to be re-replicated
is prioritized based on several factors. One is how far it is
from its replication goal. For example, we give higher prior-
ity to a chun kthat has lost two replicas than to a chun kthat
...
priority of any chun kthat is bloc king client progress.
...
dis kspace utilization, limiting active clone operations on
...
source chunkserver.
Finally, the masterrebalancesreplicas periodically: it ex-
amines the current replica distribution and moves replicas
for better dis kspace and load balancing. Also through this
...
to-chun kmappings maintained exclusively by the master.
We can also easily identify all the chun kreplicas: they are
...
component failures are common. Chun kcreation may suc-
...
Chun kreplicas may become stale if a chun kserver fails
and misses mutations to the chun kwhile it is down. For
...
increases the chun kversion number and informs the up-to-
...
able, its chun kversion number will not be advanced. The
...
the chun kversion number when it informs clients which
...
As discussed earlier, each chun kis replicated on multiple
...
its log record has been flushed to dis klocally and on all
...
or dis kfails, monitoring infrastructure outside GFS starts a
...
quently thereafter) to locate chun kreplicas and exchanges
...
dis kfailures that cause data corruption or loss on both the
read and write paths. (See Section 7 for one cause.) We
can recover from corruption using other chun kreplicas, but
...
taining checksums.
A chun kis bro ken up into 64 KB bloc ks. Each has a corre-
...
other machines. If a bloc kdoes not match the recorded
...
will clone the chun kfrom another replica. After a valid new
...
that append to the end of a chun k(as opposed to writes
...
be detected as usual when the bloc kis next read.
...
but corrupted chun kreplica from fooling the master into
...
6.1.1 Reads
Nclients read simultaneously from the file system. Each
...
125 MB/s when the 1 Gbps lin kbetween the two switches
is saturated, or 12.5 MB/s per client when its 100 Mbps
networ kinterface gets saturated, whichever applies. The
observed read rate is 10 MB/s, or 80% of the per-client
limit, when just one client is reading. The aggregate read
rate reaches 94 MB/s, about 75% of the 125 MB/s lin klimit,
...
6.1.2 Writes
Nclients write simultaneously toNdistinct files. Each
...
causeweneedtowriteeachbyteto3ofthe16chunkservers,
each with a 12.5 MB/s input connection.
The write rate for one client is 6.3 MB/s, about half of the
limit. The main culprit for this is our networ kstac k. It does
not interact very well with the pipelining scheme we use for
pushing data to chun kreplicas. Delays in propagating data
...
ited by the networ kbandwidth of the chun kservers that
store the last chun kof the file, independent of the num-
ber of clients. It starts at 6.0 MB/s for one client and drops
to 4.8 MB/s for 16 clients, mostly due to congestion and
variances in networ ktransfer rates seen by different clients.
Our applications tend to produce multiple such files con-
currently. In other words,Nclients append toMshared
...
dred engineers. A typical tas kis initiated by a human user
and runs up to several hours. It reads through a few MBs
to a few TBs of data, transforms or analyzes the data, and
writes the results bac kto the cluster. Cluster B is primarily
...
includes all chun kreplicas. Virtually all files are replicated
...
chun kversion number discussed in Section 4.5.
...
dis kbefore the server is able to answer queries. However, the
master is somewhat hobbled for a period – typically 30 to
60 seconds – until it has fetched chun klocation information
from all chunkservers.
6.2.3 Read and Write Rates
Table 3 shows read and write rates for various time pe-
riods. Both clusters had been up for about one wee kwhen
...
restart. When we too kthese measurements, B was in the
middle of a burst of write activity generating about 100 MB/s
of data, which produced a 300 MB/s networ kload because
...
(c) Record appends
Figure 3: Aggregate Throughputs.Top curves show theoretical limits imposed by our networ ktopology. Bottom curves
...
Write rate (last minute) 1MB/s 101 MB/s
Write rate (last hour) 2MB/s 117 MB/s
Writerate(sincerestart) 25 MB/s 13 MB/s
...
sources efficiently. Cluster B can support pea kread rates of
...
not a bottlenec kfor these wor kloads.
In an earlier version of GFS, the master was occasionally
a bottlenec kfor some wor kloads. It spent most of its time
...
by GFS servers. For example, GFS client code may brea ka
...
Cluster XY XY XY
0K 0.4 2.6 00 00
1B..1K 0.1 4.1 6.6 4.9 0.2 9.
1K..8K 65.2 38.5 0.4 1.0 18.9 15.
8K..64K 29.9 45.1 17.8 43.0 78.0 2.
64K..128K 0.1 0.7 2.3 1.9 <.1 4.
128K..256K 0.2 0.3 31.6 0.4 <.1 10.
256K..512K 0.1 0.1 4.2 7.7 <.1 31.
512K..1M 3.9 6.9 35.5 28.7 2.2 25.
1M..inf 0.1 1.8 1.5 12.3 0.7 2.
...
cause of the random see kwor kload.
...
Cluster XY XY XY
1B..1K <.1<.1 <.1<.1 <.1 <.
1K..8K 13.8 3.9 <.1<.1 <.1 0.
8K..64K 11.4 9.3 2.4 5.9 2.3 0.
64K..128K 0.3 0.7 0.3 0.3 22.7 1.
128K..256K 0.8 0.6 16.5 0.2 <.1 5.
256K..512K 1.4 0.3 3.4 7.7 <.1 38.
512K..1M 65.9 55.1 74.1 58.0 .1 46.
1M..inf 6.4 30.1 3.3 28.0 53.9 7.
...
Open 26.1 16.
Delete 0.7 1.
FindLocation 64.3 65.
FindLeaseHolder 7.8 13.
FindMatchingFiles 0.6 2.
All other combined 0.5 0.
...
master. Most requests as kfor chun klocations (FindLo-
...
FindMatchingFilesis a pattern matching request that sup-
...
Some of our biggest problems were dis kand Linux related.
...
to the cost offsync(). Its cost is proportional to the size
...
pages in from dis k(reader loc k) or modifies the address
space in anmmap()call (writer lock). We saw transient
timeouts in our system under light load and looked hard for
resource bottlenecks or sporadic hardware failures. Even-
tually, we found that this single loc kbloc ked the primary
networ kthread from mapping new data into memory while
the dis kthreads were paging in previously mapped data.
Since we are mainly limited by the networ kinterface rather
than by memory copy bandwidth, we worked around this by
replacingmmap()withpread()at the cost of an extra copy.
...
through a large data set or randomly see kwithin it and read
...
to implement sophisticated chun kplacement and replication
...
dis kdrives, GFS uses commodity machines as chun kservers,
...
covery. Chun kreplication allows us to tolerate chun kserver
...
data corruption at the dis kor IDE subsystem level, which
...
size and by chun kleases, which delegates authority to pri-
...
attac kproblems on the scale of the entire web.

ACKNOWLEDGMENTS
We wish to than kthe following people for their contributions
...
on a testing framewor kand security enhancements. David
...
Serverless networ kfile systems. InProceedings of the
...
Using distributed dis kstriping to provide high I/O
...
Hardin, Eri kRiedel, David Rochberg, and Jim
...
[10] Fran kSchmuc kand Roger Has kin. GPFS: A
shared-dis kfile system for large computing clusters. In
